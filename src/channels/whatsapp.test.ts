...
// Mock media module
vi.mock('../media.js', () => ({
  processInboundMedia: vi.fn(() => null),
    ...
    it('processes image with caption via processInboundMedia', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: 'Check this photo',
          attachments: [
            expect.objectContaining({ id: 'whatsapp:media:test-img' }),
          ],
        }),
      );
    });

    it('processes video with caption via processInboundMedia', async () => {
      vi.mocked(processInboundMedia).mockReturnValueOnce({
        content: 'Watch this',
        attachments: [
          {
            id: 'whatsapp:media:test-vid',
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: 'Watch this',
          attachments: [
            expect.objectContaining({ id: 'whatsapp:media:test-vid' }),
          ],
        }),
      );
    });

    it('handles voice note via processInboundMedia when it returns a result', async () => {
      vi.mocked(processInboundMedia).mockReturnValueOnce({
        content: '[Audio]',
        attachments: [
          {
            id: 'whatsapp:media:test-audio',
            filename: 'audio-123.ogg',
      ...
      expect(processInboundMedia).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          mimetype: 'audio/ogg; codecs=opus',
          mediaType: 'audio',
          ref: expect.objectContaining({ downloadType: 'ptt' }),
        }),
      );
      expect(opts.onMessage).toHaveBeenCalledWith(
        'registered@g.us',
        expect.objectContaining({
          content: '[Audio]',
          attachments: [
            expect.objectContaining({ id: 'whatsapp:media:test-audio' }),
          ],
        }),
      );
    });

    it('skips voice note when processInboundMedia returns null and no text', async () => {
      vi.mocked(processInboundMedia).mockReturnValueOnce(null);
      ...
      // Skipped — processInboundMedia returned null and no text content
      expect(opts.onMessage).not.toHaveBeenCalled();
    });
