...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  const prompt = datePrefix() + formatMessages(missedMessages, TIMEZONE);
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          const formatted = datePrefix() + formatMessages(allPending, TIMEZONE);
  ...
  startIpcWatcher({
    ...
    sendMedia: async (jid, filePath, options) => {
      ...
      if (channel.sendMedia) {
        ...
        // Fallback: send caption or filename as text
